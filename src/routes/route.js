...
router.get('/hello', function (req, res) {
    ...
    //let values=_.chunk(months, 3)
    console.log(_.chunk(months, 3))
    console.log("get lasst 9 odd function using tail " +_.tail(oddno))
    console.log("merge 5 array with unique no: "+ _.union(oddno,arry,arry2,arry3,arry4))
...
router.get('/sol1',function(req, res){
    ...
    num.forEach(add)
    let missingNumber=0
    missingNumber=lastNum*(lastNum+1)/2-sumofarray
...
router.get('/sol2',function(req, res){
    let num=[33, 34, 35, 37, 38]
    let lastNum=_.last(num)
    let firstNum=_.head(num)
    let sumofarray=0
    function add(x){
        sumofarray+=x
    }
    num.forEach(add)
    let missingNumber=0
    missingNumber=(num.length+1)*(lastNum+firstNum)/2-sumofarray
    console.log(missingNumber)
    res.send('done')
})
